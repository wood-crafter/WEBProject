    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<ProductInCart> productsInCart = (ArrayList<ProductInCart>) session.getAttribute("productsInCart");
            for (int i = 0; i < productsInCart.size(); i++) {
                if (request.getParameter(productsInCart.get(i).getId()) != null) {
                    int newQuantity = Integer.valueOf(request.getParameter(productsInCart.get(i).getId()));
                    productsInCart.get(i).setQuantity(newQuantity);
